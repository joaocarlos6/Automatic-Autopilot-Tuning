%% SimulationSensorConfigUTAIL7P.m
...
% Load Cell based WOW with Switch/Latch Characteristics
% Hysteresis
SensorModel.WOW.Cell.loadThresholdUpper_N   = 6.5; 
SensorModel.WOW.Cell.loadThresholdLower_N   = 3.5; 
% Hysteresis Alternate 
SensorModel.WOW.Cell.loadThresholdUpper2_N  = 6.5; 
SensorModel.WOW.Cell.loadThresholdLower2_N  = 5;
% Timing
SensorModel.WOW.Cell.persistanceTime_s      = 0.0;
SensorModel.WOW.Cell.longLatchTime_s        = 0.40; % Long (primary) latch timer. If dual timer mode disabled, this is the parameter used for single timer.
SensorModel.WOW.Cell.shortLatchTime_s       = 0.30; % Short (secondary) latch timer. Used when dual timer mode enabled
SensorModel.WOW.Cell.timeout                = 0.40; % S, timeout period for single contact (Load 2)
% Enable Flags
